...
// post request handling for uploading user unfollow request details to mongoDB
app.post("/unfollowupdate", (req, res) => {
    console.log("post request made for unfollow");
    UserDetails.find({ username: req.user.username }, (err, data) => {
        if (err) console.log(err);
        else {
            let myFollowing = data[0].following; //now myFollowing contains all the initial following
            console.log(myFollowing, "before unfollowing", req.body.username);
            myFollowing = myFollowing.filter(item => item != req.body.username);

            console.log(myFollowing, "after unfollowing", req.body.username);
            ...
            UserDetails.updateOne({ username: req.user.username }, { following: myFollowing }, function (err, result) {
                if (err) {
                    ...
                    res.redirect(url.format({
                        pathname: `/home`,
                        query: {
                            message: `You unfollowed ${req.body.username} successfully!.`
                        }
                    }));
                }
            });
            ...
            if (err)
                ...
                UserDetails.find({}, async (err, totalData) => {
                    if (err)
                        ...
                        followUsers = followUsers.filter(item => item.username != data[0].username);
                        followUsers = followUsers.filter(x => !data[0].following.includes(x.username));
                        console.log(followUsers, "followUsers");
                        let following = totalData.filter(item => !followUsers.includes(item) && (item.username != data[0].username));
